...
from src.lerobot.cameras import (  # noqa: F401
    CameraConfig,  # noqa: F401
)
from src.lerobot.cameras.opencv.configuration_opencv import OpenCVCameraConfig  # noqa: F401
from src.lerobot.cameras.reachy2_camera.configuration_reachy2_camera import Reachy2CameraConfig  # noqa: F401
...
from src.lerobot.datasets.image_writer import safe_stop_image_writer
from src.lerobot.datasets.lerobot_dataset import LeRobotDataset
from src.lerobot.datasets.pipeline_features import aggregate_pipeline_dataset_features, create_initial_features
...
from src.lerobot.policies.utils import make_robot_action
...
from src.lerobot.processor.rename_processor import rename_stats
...
from src.lerobot.teleoperators.keyboard.teleop_keyboard import KeyboardTeleop
...
class DatasetRecordConfig:
    ...
    def __post_init__(self):
        # For walker_s2_sim robot, single_task can be set later via the task parameter
        # So we don't require it to be set here
        pass
        ...
        if hasattr(self.robot, "task_name") and self.task:
            ...
            if hasattr(self.robot, "load_from_yaml"):
                try:
                    ...
                    # Update dataset single_task if not provided
                    if self.dataset.single_task is None and hasattr(self.robot, "task_cfg"):
                        task_desc = self.robot.TASK_CONFIGS.get(self.task, {}).get("desc", self.task)
                        self.dataset.single_task = task_desc
...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        process_dt_s = time.perf_counter() - start_loop_t
        sleep_time_s: float = 1 / fps - process_dt_s
        precise_sleep(max(sleep_time_s, 0.0))

        control_dt_s = time.perf_counter() - start_loop_t
        control_fps = 1 / control_dt_s if control_dt_s > 0 else float("inf")
        log_control_info(control_fps)   

        timestamp = time.perf_counter() - start_episode_t
    total_elapsed_s = max(time.perf_counter() - start_episode_t, 1e-9)
    logging.info(
        "control_loop 结束，共执行 %d 次循环，总平均频率 %.1fHz，目标频率 %dHz",
        loop_count,
        loop_count / total_elapsed_s,
        fps,
    )
